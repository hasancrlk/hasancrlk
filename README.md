<div align="center">

# Merhaba, ben Hasan Carlak 👋
# Hi, I'm Hasan Carlak 👋

<em>Bilgisayar Mühendisliği Öğrencisi (3. Sınıf) — Düzce Üniversitesi</em>
<br/>
<em>Computer Engineering Student (3rd Year) — Düzce University</em>
<br/><br/>
📍 İstanbul, Türkiye

</div>

---

### 🇹🇷 Hakkımda
Düzce Üniversitesi Bilgisayar Mühendisliği bölümünde 3. sınıf öğrencisiyim. Veri yapıları, bilgisayar ağları, veritabanı yönetim sistemleri ve nesne yönelimli programlama derslerini tamamladım; eğitim dilim Türkçe olmasına rağmen destekli olarak %100 İngilizce hazırlık eğitimini bitirdim.

Siber Vatan Programı kapsamında Linux sistem yönetimi, ağ güvenliği ve sızma testleri üzerine yoğunlaştırılmış teknik eğitim alıyorum. MEKATEK Topluluğu'nda 2+ yıldır İHA (insansız hava aracı) takımının yazılım ekibinde, Python ve OpenCV kütüphaneleriyle görüntü işleme algoritmaları üzerinde çalışıyorum.

### 🇬🇧 About Me
I'm a 3rd-year Computer Engineering student at Düzce University. I've completed coursework in data structures, computer networks, database management systems, and object-oriented programming, and finished a full English preparatory program alongside my Turkish-language studies.

Through the Siber Vatan Program I'm getting intensive hands-on training in Linux system administration, network security, and penetration testing. For 2+ years I've been part of the MEKATEK Community's UAV team, building image-processing algorithms with Python and OpenCV.

---

### 🚀 Deneyim / Experience

**🛡️ Siber Vatan Programı** *(Kalkınma Ajansları)*
TR: Siber güvenlik alanında derinleşmek için seçildiğim bu programda Linux sistem yönetimi, ağ güvenliği ve sızma testleri üzerine teknik eğitim ve uygulama süreçlerine devam ediyorum.
EN: Selected for this cybersecurity-focused program — training intensively in Linux administration, network security, and penetration testing.

**🛩️ MEKATEK Topluluğu — İHA Takımı** *(2+ Yıl)*
TR: İnsansız hava araçları takımının yazılım ekibinde görev alarak, Python ve OpenCV kütüphaneleri ile görüntü işleme algoritmaları üzerine pratik çalışmalar yürütüyorum.
EN: Member of the UAV team's software group, developing image-processing algorithms with Python and OpenCV.

---

### 🛠️ Teknolojiler / Tech Stack

**Yazılım ve Programlama / Programming**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)

**Siber Güvenlik ve Sistem / Security & Systems**

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Network Security](https://img.shields.io/badge/Network%20Security-2E7D32?style=for-the-badge&logo=shieldsdotio&logoColor=white)
![Pentesting](https://img.shields.io/badge/Pentesting-B71C1C?style=for-the-badge&logo=hackaday&logoColor=white)

**Web ve Veritabanı / Web & Database**

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

**Görüntü İşleme / Image Processing**

![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![YOLO](https://img.shields.io/badge/YOLO-111F68?style=for-the-badge)

---

### 📌 Öne Çıkan Projeler / Featured Projects

**🔹 Web Tasarım — Marmara Bölgesi**
TR: Marmara bölgesini tanıtan bir web tasarım projesi.
EN: A web design project showcasing the Marmara region.
🔗 [Repo'yu görüntüle / View repo](https://github.com/hasancrlk/Web-tasar-m-marmara-b-lgesi)

**🔹 Converter**
TR: Beden dilini Türkçeye çeviren bir araç üzerinde çalışıyorum.
EN: A tool I'm building to translate body language into Turkish.
🔗 [Repo'yu görüntüle / View repo](https://github.com/hasancrlk/Converter)

**🔹 Coffee POS**
TR: React ile geliştirdiğim bir kafe/coffee shop satış noktası (POS) uygulaması.
EN: A coffee shop point-of-sale (POS) application built with React.
🔗 [Repo linki eklenecek / Repo link coming soon]

---

### 📈 GitHub İstatistikleri / GitHub Stats

<div align="center">

![Hasan's GitHub stats](https://github-readme-stats.vercel.app/api?username=hasancrlk&show_icons=true&theme=default)

</div>

---

### 📫 İletişim / Contact

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hasan-carlak-294798253/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hasan.crlk@outlook.com)

</div>

